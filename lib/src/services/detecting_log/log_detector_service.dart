...
/// All the command codes that can be sent and received between [LogDetector] and
/// [_DetectorServer].
enum _Codes {
  init,
  busy,
...
/// A command sent between [LogDetector] and [_DetectorServer].
class _Command {
...
class LogDetector {
  static const String _modelPath =
      'assets/models/log_model/best_float32.tflite';
  static const String _labelPath = 'assets/models/log_model/labelmap.txt';

  LogDetector._(this._isolate, this._interpreter, this._labels);
  ...
  static Future<LogDetector> start() async {
    ...
    final LogDetector result = LogDetector._(
      isolate,
      await _loadModel(),
      await _loadLabels(),
    );
...
/// The portion of the [LogDetector] that runs on the background isolate.
///
...
class _DetectorServer {
  ...
  List<RecognitionModel> postProcessing(List<List<List<List<double>>>> output) {
    ...
    //     'Shape of output: [${output[0]!.length}, ${output[0]![0].length}, ${output[0]![0][0].length}]');
  ...
  List<List<List<List<double>>>> _runInference(
    ...
    // print('Output shape: ${outputShape[0]}, ${outputShape}');
    // Set input tensor [1, 3, 244, 244]
    //final input = [imageMatrix];
    //print('Shape of imageMatrix: [${input.length}, ${input[0].length}]');
    // Set output tensor
    // Locations: [1029, 4]
    // Classes: [1029, 1],
    final output = {
    ...
    // print(
    //     'Shape of imageMatrix: [${imageMatrix.length}, ${imageMatrix[0].length}, ${imageMatrix[0][0].length}, ${imageMatrix[0][0][0].length}]');
    // print(
    //     '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
